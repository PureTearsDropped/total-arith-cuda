...
### 1.5 配線正規形 — 三値契約 (2026-07-23)

**正規形**: 配線 (U,V,W) の係数は 三値 {−1,0,+1}、二進有理の倍率は べき2対角
(指数シフト = 乗算器なし) に括り出す。register 済み配線 18/18 (CD 系・Clifford・
巡回/negacyclic・二面体・行列積・Strassen/Karatsuba/Gauss) がこの形に収まることを
`wiring_registry.self_test` の三値門番が恒久検査する。

買うもの:
1. **配線段の完全厳密性** — U,V,W の適用が 経路+符号+加算 だけになり、係数の丸めが
   どのバックエンドにも存在しない。4実行機のビット一致 (§7) はこの上に立つ:
   適合性は ⊙ と加算順序だけに依存する。
2. **R = 正直な請求書** — 「真の乗算は R 本」は U,V,W が乗算フリーのときだけ正直。
3. **フラグの素通り** — |係数|=1 なので GE/LE の大きさ主張が配線を無スケールで通過する
   (`group_mul` のパターン則が sign(T) だけ見て済む理由)。
4. **SD 算術との合流** — 符号付き桁 {−1,0,1} と同じアルファベット。三値配線 × SD
   データパスでは係数乗算 = 桁ごとの符号反転で、線形段全体が桁上げフリーの加算木。

**入場規則** (係数の3階級):
- **±1 (符号置換)**: どの精度でも同一配線で厳密。CD/Clifford/群代数/Strassen 族。
- **二進有理**: 三値 + べき2シフトで有限桁・厳密。まだ精度から独立。
- **無理数**: CSD (正準符号桁) 展開で常に三値化可能だが、**打ち切り桁数が配線に
  焼き込まれる** — 代金は精度比例 (実測: √2/2 は f32級で 加算×10・f64級で ×20、
  対して 3/4 は恒久×2)。配線が初めて誤差を持つため、**誤差票 (係数の真値区間) を
  レジストリに添えて宣言する**こと。IEEE が定数丸めを黙って行うのに対し、宣言付き
