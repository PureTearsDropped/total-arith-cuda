...
@inline function split18(v::Float32)
    ...
    r = v - h
    neg = r < 0f0
    h = ifelse(neg, h - RADIX, h)
    r = ifelse(neg, r + RADIX, r)
    return h * IRADIX, r
end
...
@inline function splitbit(d::Float32, s::Int)
    ...
    h = ifelse(neg, h - p, h)
    r = ifelse(neg, r + p, r)
    return h * IPOW2[s + 1], r
end

# ---------------------------------------------------------------------------------------
# 2. digit vectors (little-endian: v[1] least significant) — still Float32 only
# ---------------------------------------------------------------------------------------
"""carry ripple with floor semantics (negative entries become borrows); returns the carry out."""
function carry_norm!(v::Vector{Float32})
    c = 0f0
...
"""exact schoolbook product; column sums < 2·min(na,nb)·2^18 < 2^24 (exact) needs min(na,nb) ≤ 31."""
function mul_digits(a::Vector{Float32}, b::Vector{Float32})
    na, nb = length(a), length(b)
    min(na, nb) ≤ 31 || error("mul_digits: column sums would leave the exact range")
    c = zeros(Float32, na + nb)
    @inbounds for i in 1:na
        ai = a[i]; ai == 0f0 && continue
        for j in 1:nb
            h, l = mulsplit18(ai, b[j])
            c[i + j - 1] += l
            c[i + j]     += h
        end
    end
...
"""left shift by s bits (0 ≤ s < 18); returns length(d)+1 digits."""
function shl_bits(d::Vector{Float32}, s::Int)
    n = length(d); out = zeros(Float32, n + 1)
    if s == 0
        @inbounds for i in 1:n; out[i] = d[i]; end
        return out
    end
    p = POW2[s + 1]; carry = 0f0
    @inbounds for i in 1:n
        q, r = splitbit(d[i], DB - s)          # d = q·2^(18−s) + r
        out[i] = r * p + carry                 # r·2^s + q of the digit below  (< 2^18, exact)
        carry = q
    end
    out[n + 1] = carry
    return out
...
"""left shift by n ≥ 0 bits (digits + bits), exact."""
function shl_by(d::Vector{Float32}, n::Int)
    n ≥ 0 || error("shl_by: negative")
    ds = fld(n, DB); s = n - DB * ds
    e = shl_bits(d, s)
    ds == 0 ? e : vcat(zeros(Float32, ds), e)
end
...
function place(d::Vector{Float32}, shift::Int, L::Int)
    ds = fld(shift, DB); s = shift - DB * ds
    e = shl_bits(d, s)
    w = zeros(Float32, L); st = false
...
function cmp_digits(a::Vector{Float32}, b::Vector{Float32})
    length(a) == length(b) || error("cmp_digits: lengths")
...
function topbit(d::Vector{Float32})
    @inbounds for i in length(d):-1:1
        d[i] != 0f0 && return DB * (i - 1) + exponent(d[i])
    end
    return -1
end

"""add a small integer to a non-negative digit vector (in place; may append a digit)."""
function add_int!(d::Vector{Float32}, v::Int)
    d[1] += Float32(v)
    co = carry_norm!(d)
    co == 0f0 && return d
    co > 0f0 ? push!(d, co) : error("add_int!: negative result")
    return d
end

isodd_digits(d::Vector{Float32}) = splitbit(d[1], 1)[2] == 1f0
lowbits(d::Vector{Float32}, s::Int) = splitbit(d[1], s)[2]

"""exact (da·2^ea) − (db·2^eb) → (neg, digits, e0)."""
function wide_sub(da::Vector{Float32}, ea::Int, db::Vector{Float32}, eb::Int)
    e0 = min(ea, eb)
    A = shl_by(da, ea - e0); B = shl_by(db, eb - e0)
    L = max(length(A), length(B))
    A = vcat(A, zeros(Float32, L - length(A))); B = vcat(B, zeros(Float32, L - length(B)))
    c = cmp_digits(A, B)
    c == 0 && return (false, zeros(Float32, 1), e0)
    neg = c < 0
    if neg; A, B = B, A; end
    r = A .- B
    co = carry_norm!(r)
...
function wide_cmp(da::Vector{Float32}, ea::Int, db::Vector{Float32}, eb::Int)
    neg, r, _ = wide_sub(da, ea, db, eb)
    all(==(0f0), r) && return 0
    return neg ? -1 : 1
end
...
to_vec(x::T) where {T<:MFloat} = (collect(x.d), Int(x.ex) - DB * ndig(T) + 1)
...
function round_pack(::Type{T}, neg::Bool, d::Vector{Float32}, e0::Int, sticky::Bool, Pr::Int) where {T<:MFloat}
    K = ndig(T)
    L = length(d)
    while L ≥ 1 && d[L] == 0f0; L -= 1; end
    if L == 0
        sticky && error("round_pack: zero window with a non-zero sticky (window too narrow)")
        return zero(T)
    end
    d = d[1:L]
    t = e0 + DB * (L - 1) + exponent(d[L])        # leading bit
    u = t - Pr + 1                                 # ulp position
    if u > e0                                      # something to round inside the window
        iu = fld(u - e0, DB) + 1; bu = mod(u - e0, DB)
        q, r = splitbit(d[iu], bu)                 # d[iu] = q·2^bu + r
        below(i) = sticky || any(!=(0f0), @view d[1:i])
        if bu ≥ 1
            half = POW2[bu]                        # 2^(bu−1)
            cmp = r > half ? 1 : (r < half ? -1 : (below(iu - 1) ? 1 : 0))
        elseif iu ≥ 2
            x = d[iu - 1]; half = POW2[DB]
            cmp = x > half ? 1 : (x < half ? -1 : (below(iu - 2) ? 1 : 0))
        else
            sticky && error("round_pack: round bit below the window")
            cmp = -1
        end
        d[iu] = q * POW2[bu + 1]
        @inbounds for i in 1:iu - 1; d[i] = 0f0; end
        if cmp > 0 || (cmp == 0 && splitbit(q, 1)[2] == 1f0)
            d[iu] += POW2[bu + 1]
            co = carry_norm!(d)
            co == 0f0 || push!(d, co)
            L = length(d)
            while d[L] == 0f0; L -= 1; end
            t = e0 + DB * (L - 1) + exponent(d[L])
        end
    else
        sticky && error("round_pack: round bit below the window")
    end
    # pack: leading bit → bit 17 of digit K   (new LSB weight t − 18K + 1)
    w, st = place(d, e0 - (t - DB * K + 1), K)
    ...
    return T(neg, t, ntuple(i -> w[i], Val(K)), 0x00)
end
...
function (::Type{T})(x::Float64) where {T<:MFloat}
    ...
    round_pack(T, den < 0, digits_of(abs(num)), pow, false, prec(T))
end
...
function (::Type{T})(x::Integer) where {T<:MFloat}
    x == 0 && return zero(T)
    round_pack(T, x < 0, digits_of(abs(big(x))), 0, false, prec(T))
end
...
function add_core(::Type{T}, a::T, b::T, flipb::Bool, Pr::Int) where {T<:MFloat}
    K = ndig(T); L = K + 2
    negb = b.neg ⊻ flipb
    if iszero(a)
    ...
    wA = vcat(zeros(Float32, 2), collect(A.d))               # L digits
    wB, stB = place(collect(B.d), 2 * DB - Δ, L)
    if negA == negB
        s = wA .+ wB
        co = carry_norm!(s); push!(s, co)
        return round_pack(T, negA, s, e0w, stB, Pr)
    end
    if Δ == 0
        c = cmp_digits(wA, wB)
        c == 0 && return zero(T)
    ...
    s = wA .- wB
    stB && (s[1] -= 1f0)                                     # the sticky is a borrow of one window-ulp …
    co = carry_norm!(s)
    co == 0f0 || error("add_core: borrow out")
    return round_pack(T, negA, s, e0w, stB, Pr)              # … and stays sticky (remainder ∈ (0, ulp_w))
end

function mul_core(::Type{T}, a::T, b::T, Pr::Int) where {T<:MFloat}
    (iszero(a) || iszero(b)) && return zero(T)
    K = ndig(T)
    p = mul_digits(collect(a.d), collect(b.d))
...
const CORRECTION_STATS = Dict{Symbol,Int}(:div => 0, :sqrt => 0)   # max residual fix-ups seen

"""|a| / |b| correctly rounded at P (b ≠ 0), with the sign; flag = saturation only."""
function div_core(::Type{T}, a::T, b::T) where {T<:MFloat}
    P, K = prec(T), ndig(T)
    e0s = 1 - DB * K
    ...
    dq, eq = to_vec(qh)
    Q, _ = place(dq, eq + P, K + 1)                    # Q = ⌊q̂·2^P⌋   (weight 2^-P)
    da = collect(a.d); db = collect(b.d)
    resid() = (pm = mul_digits(Q, db); wide_sub(da, e0s, pm, e0s - P))   # R = asig − Q·m·2^-P
    negR, dR, eR = resid()
    fix = 0
    while negR                                          # Q too large
        add_int!(Q, -1); negR, dR, eR = resid(); fix += 1; fix > 8 && error("div_core: candidate too far (−)")
    end
    while wide_cmp(dR, eR, db, e0s - P) ≥ 0             # R ≥ m·2^-P → Q too small
        add_int!(Q, 1); negR, dR, eR = resid(); fix += 1; fix > 8 && error("div_core: candidate too far (+)")
    end
    CORRECTION_STATS[:div] = max(CORRECTION_STATS[:div], fix)
    # now Q·2^-P ≤ q_exact < (Q+1)·2^-P with remainder R ∈ [0, m·2^-P)
    if topbit(Q) < P                                    # q < 1 : ulp = 2^-P, round bit = 2R vs m·2^-P
        c = wide_cmp(dR, eR + 1, db, e0s - P)
        (c > 0 || (c == 0 && isodd_digits(Q))) && add_int!(Q, 1)
    else                                                # q ≥ 1 : ulp = 2^(1-P) = two grid steps
        if isodd_digits(Q)
            if all(==(0f0), dR)                          # exact midpoint between Q−1 and Q+1 → the one ≡ 0 (mod 4)
                add_int!(Q, lowbits(Q, 2) == 1f0 ? -1 : 1)
            else
                add_int!(Q, 1)
            end
        end
    end
    round_pack(T, a.neg ⊻ b.neg, Q, -P + Int(a.ex) - Int(b.ex), false, P)
end

"""√|a| correctly rounded at P (a ≠ 0); flag = saturation only."""
function sqrt_core(::Type{T}, a::T) where {T<:MFloat}
    P, K = prec(T), ndig(T)
    odd = isodd(Int(a.ex))
    ...
    S, _ = place(ds, es - g, K + 1)                     # S = ⌊ŝ / 2^g⌋
    dm, em = to_vec(mp)
    resid() = (ss = mul_digits(S, S); wide_sub(dm, em, ss, 2 * g))   # R = m' − S²·2^{2g}
    negR, dR, eR = resid()
    fix = 0
    while negR
        add_int!(S, -1); negR, dR, eR = resid(); fix += 1; fix > 8 && error("sqrt_core: candidate too far (−)")
    end
    while true                                          # (S+1)² ≤ m'/2^{2g}  ⟺  R ≥ 2S+1
        t = shl_bits(S, 1); add_int!(t, 1)
        wide_cmp(dR, eR, t, 2 * g) ≥ 0 || break
        add_int!(S, 1); negR, dR, eR = resid(); fix += 1; fix > 8 && error("sqrt_core: candidate too far (+)")
    end
    CORRECTION_STATS[:sqrt] = max(CORRECTION_STATS[:sqrt], fix)
    f = shl_bits(S, 2); add_int!(f, 1)                  # round up ⟺ 4R > 4S+1  (never equal)
    wide_cmp(dR, eR + 2, f, 2 * g) > 0 && add_int!(S, 1)
    round_pack(T, false, S, g + exr, false, P)
end
...
function Base.:/(a::T, b::T) where {T<:MFloat}
    bz = iszero(b)
    r = (bz || iszero(a)) ? zero(T) : div_core(T, a, b)          # a/0 = 0
    fin = a.flag | b.flag
    nb = (fin & (GE | LE)) > 0
...
function self_test(; n = 1000, seed = 20260903)
    ...
    println("MultiF32Ref (Vector reference) self-test — digits: $DB bits in Float32, radix 2^$DB, K ≤ 31")
    ...
    let bad = 0
        ...
        for _ in 1:2000
            la = rand(rng, 1:31); lb = rand(rng, 1:31)
            A = rand(rng, big(0):big(2)^(DB * la) - 1); B = rand(rng, big(0):big(2)^(DB * lb) - 1)
            da = vcat(digits_of(A), zeros(Float32, la - length(digits_of(A))))
            dbv = vcat(digits_of(B), zeros(Float32, lb - length(digits_of(B))))
            p = mul_digits(da, dbv)
            val = sum(big(Int(p[i])) << (DB * (i - 1)) for i in eachindex(p))
            val == A * B || (bad += 1)
            sh = rand(rng, 0:60)
            w, st = place(da, sh, la + 4)
            wv = sum(big(Int(w[i])) << (DB * (i - 1)) for i in eachindex(w))
            (wv == (A << sh) && !st) || (bad += 1)
            w2, st2 = place(da, -sh, la)
    ...
    for T in (F128, F256, F512)
        ...
        for _ in 1:200
            ...
            check("odd-exp-sqrt", sqrt(T(ldexp(BigFloat(3), 2 * e + 1))), oracle(T, sqrt, T(ldexp(BigFloat(3), 2 * e + 1))), log)
        end
        ...
        println("    vs MPFR: n=$n random ×(add,sub,mul,div,sqrt) + 200×15 adversarial + edges → mismatches = $nbad" *
                "   [max residual fix-ups: div $(CORRECTION_STATS[:div]), sqrt $(CORRECTION_STATS[:sqrt])]")
        println("    time/op: add $(round(1e6*t_add/(2n), digits=1)) µs, mul $(round(1e6*t_mul/n, digits=1)) µs, " *
                "div $(round(1e6*t_div/n, digits=1)) µs, sqrt $(round(1e6*t_sqrt/n, digits=1)) µs  (Vector-based reference)")
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    MultiF32Ref.self_test(n = n) || exit(1)
end
